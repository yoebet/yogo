...
let GameCenter=require('../src/game_ui/game_center');

var viewer=new GameCenter('.game-viewer:first');
viewer.init();
},{"../src/game_ui/game_center":19}],2:[function(require,module,exports){
...
},{"../util":21,"./board_drawer":3,"./board_setting":4,"./board_transformer":5,"./coordinate_manager":6,"./label_manager":7,"./marker_manager":8,"./stone_manager":9}],3:[function(require,module,exports){
...
class MarkerManager {
	...
	setMarker(coor,markerType){
		...
		if(marker){
			util.logWarn('point ('+coor.x+','+coor.y+') has a marker','setMarker');
		...
		if(!markerTemplate){
			util.logError('markerTemplate '+markerKey+' not found','setMarker');
			return;
		}
		...
		// util.logInfo('marker '+markerKey+' from template','setMarker');
		...
		if(markerType=='CR'||markerType=='TW'||markerType=='TB'){
			...
			util.logWarn('do translate','setMarker');
	...
	removeMarker(coor){
		...
		if(!pointStatus||!pointStatus.marker){
			util.logWarn('no marker at ('+coor.x+','+coor.y+')','removeMarker');
			return;
		}
...
},{"../board/board":2,"../game_model/model":15,"../util":21,"./game_marker":11,"./game_player":12,"./node_navigator":13,"./position_builder":14}],11:[function(require,module,exports){
let util=require("../util");
...
class GameMarker {
	...
	setCurrentNodeMarkers() {
		...
		if (curNode.hasMarks()) {
			...
			for (let mi = 0; mi < markerTypes.length; mi++) {
	...
	setMarkers(points, marker, processRange) {
		var board = this.board;
		if (processRange) {
	...
	setMarkBranchPoints(markBranchPoints) {
...
},{"../game_model/model":15,"../util":21,"./position_builder":14}],13:[function(require,module,exports){
...
class SgfExport {
	...
	static exportSgf(gameModel) {
		let exp = new SgfExport(gameModel);
		let sgf = exp.generateSgf();
		return sgf;
	}
}


module.exports=SgfExport
...
},{"../util":21,"./model":15}],18:[function(require,module,exports){
...
},{"../board/board":2,"../game/game":10,"../game_model/model":15,"../game_model/sgf_export":16,"../game_model/sgf_parser":17,"../util":21,"./event_handler":18,"./game_tree":20}],20:[function(require,module,exports){
let util=require("../util");
...
class GameTree {
	...
	render() {
		...
		var $treeNodesTmpl = ets.treeNodes;
		var $treeNodeTmpl = ets.treeNode;

		var $tree = ets.gameTree.clone();
		// $tree.attr('id', 't' + util.nextuid());
	...
	createVariation(variation) {
		var ets = this.elementTemplates;
		var $variationTmpl = ets.variation;
		var $variationHeadTmpl = ets.variationHead;
		var $treeNodesTmpl = ets.treeNodes;

		var $v = $variationTmpl.clone().attr('id', variation.id);
		var $variationHead = $variationHeadTmpl.clone();
	...
	setVariationHead(variation, $variationHead) {
		if (!$variationHead) {
	...
	createNodeGroup(moveNumberFrom) {
		var ets = this.elementTemplates;
		var $nodeGroupTmpl = ets.nodeGroup;
		var $nodeGroupHeadTmpl = ets.nodeGroupHead;
		var $treeNodesTmpl = ets.treeNodes;
	...
	removeLastNode(node, newVariation0) {
		if (!node.isVariationLastNode()) {
			return false;
		}
