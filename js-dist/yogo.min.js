...
this.labelManager,"setBranchPointOnclickHandler setLabel setLabels removeLabel removeAllLabels removeBranchPointLabels".split(" "))}
Board.getDefaultBoardSetting=function(a){(isNaN(a)||5>a||51<a)&&yogo.logError("wrong board size: "+a,"board");var b=20,c=3,d=3,e=12,f=12,g=2*e+2*c+f+d,h=b*(a-1)+g;if(19!=a)var g=18*b+g,l=h/g,h=g,b=b/l,c=c/l,d=d/l,e=e/l,f=f/l;var g=c+f+e+d,g={x:g,y:g},c=c+f/2,d={x:{basey:c,type:"A"},y:{basex:c,type:"1"},fontSize:b/2,coordinatePadding:d,coordinateWidth:f},f={outerBorderLine:.03*b,borderLine:.025*b,star:.045*b,stone:.02*b,stoneSpacing:.01*b},c={fontSize:b/2+3},l={fontSize:b/2+1},k=[];1==a%2&&(k.push({x:(a-
1)/2,y:(a-1)/2}),13<=a&&(k.push({x:3,y:(a-1)/2}),k.push({x:a-4,y:(a-1)/2}),k.push({x:(a-1)/2,y:3}),k.push({x:(a-1)/2,y:a-4})));if(11<=a)k.push({x:3,y:3}),k.push({x:3,y:a-4}),k.push({x:a-4,y:3}),k.push({x:a-4,y:a-4});else if(8==a||9==a)k.push({x:2,y:2}),k.push({x:2,y:a-3}),k.push({x:a-3,y:2}),k.push({x:a-3,y:a-3});return{viewBoxSize:h,boardSize:a,starPoints:k,boardOrigin:g,gridWidth:b,boardOuterEdge:e,strokes:f,labels:c,coordinate:d,moveNumbers:l}};
Board.prototype={drawBoard:function(){var a=this.boardSetting,b=this.paper,c=a.viewBoxSize;yogo.logInfo("viewBox size:"+c,"board");b.setViewBox(0,0,c,c);var d=a.boardSize,c=a.boardOrigin,e=a.gridWidth,f=a.boardOuterEdge,g=a.strokes,h=e*(d-1)+2*f,f=b.rect(c.x-f,c.y-f,h,h);f.attr({"stroke-width":0});f.attr({fill:"#DCB35C"});b.rect(c.x,c.y,e*(d-1),e*(d-1)).attr({"stroke-width":g.outerBorderLine});for(var l=h="",f=0;f<d-2;f++)h+="M"+c.x+" "+(c.y+e+e*f)+"H"+(c.x+e*(d-1)),l+="M"+(c.x+e+e*f)+" "+c.y+"V"+
(c.y+e*(d-1));b.path(h).attr({"stroke-width":g.borderLine});b.path(l).attr({"stroke-width":g.borderLine});a=a.starPoints;for(f=0;f<a.length;f++)d=a[f],b.circle(c.x+e*d.x,c.y+e*d.y,g.star).attr({fill:"black"});this.drawCoordinate()},clearBoard:function(){for(statusKey in this.pointStatusMap){var a=this.pointStatusMap[statusKey];a.color&&this.removeStone(a.coor)}this.removeAllMarkers();this.removeAllLabels();this.markCurrentMove(null)},zoomBoard:function(a){this.zoomMode=a;var b=this.boardSetting.viewBoxSize,
...
Board.Label=function(a){this.board=a;this.boardSetting=a.boardSetting;this.paper=a.paper;this.pointStatusMap=a.pointStatusMap;this.coordinateManager=a.coordinateManager;this.branchPointOnclickHandler;this.labelPoints=[];this.branchPoints=[]};
Board.Label.prototype={setBranchPointOnclickHandler:function(a){this.branchPointOnclickHandler=a},setLabel:function(a,b,c){if(b)if(2<b.length)yogo.logError("label too long: "+b,"setLabel");else{var d="x"+a.x+"y"+a.y,e=this.pointStatusMap[d],f=null,g=null;e?(f=e.color,g=e.label):(e={},this.pointStatusMap[d]=e);g&&e.labelElement.remove();d="black";"B"==f&&(d="white");"branch_point"===c&&(d="blue",f&&yogo.logWarn("branch point ("+a.x+","+a.y+") has a stone","setLabel"));f=this.boardSetting.labels.fontSize;
2==b.length&&(f-=1);g=this.coordinateManager.boardCoorToViewBoxCoor(a);f=this.paper.text(g.x,g.y,b).attr({"font-size":f,fill:d});e.labelElement=f;e.label=b;if("branch_point"===c){this.branchPoints.push(a);f.attr({cursor:"pointer"});f.data("branch",b);var h=this.branchPointOnclickHandler;f.click(function(a){"function"===typeof h&&h(this.data("branch"))})}else this.labelPoints.push(a)}else yogo.logError("label missing: ("+a.x+","+a.y+")","setLabel")},setLabels:function(a){for(var b=0;b<a.length;b++){var c=
a[b];this.setLabel(c,c.label)}},removeLabel:function(a){(a=this.pointStatusMap["x"+a.x+"y"+a.y])&&a.label&&(a.labelElement.remove(),a.labelElement=null,a.label=null)},removeAllLabels:function(){for(;0<this.labelPoints.length;)this.removeLabel(this.labelPoints.pop())},removeBranchPointLabels:function(){for(;0<this.branchPoints.length;)this.removeLabel(this.branchPoints.pop())}};
Board.Marker=function(a){this.board=a;this.boardSetting=a.boardSetting;this.paper=a.paper;this.pointStatusMap=a.pointStatusMap;this.coordinateManager=a.coordinateManager;this.templateMarkerPoint={x:-this.boardSetting.gridWidth,y:-this.boardSetting.gridWidth};this.currentMoveMarker=null;this.markerPoints=[];this.markerTemplates=this.setupMarkerTemplates()};
Board.Marker.prototype={setupMarkerTemplates:function(){var a=this.paper,b=this.boardSetting.gridWidth,c=this.boardSetting.strokes,d=this.templateMarkerPoint,e=a.circle(d.x,d.y,.23*b).attr({"stroke-width":.1*b}),f=e.clone().attr({stroke:"white"}),g=.4*b,g=a.rect(d.x,d.y,g,g).attr({"stroke-width":0,fill:"white"}),h=g.clone().attr({fill:"black"}),l=.5*b,k=l*Math.sin(Math.PI/3),m=l/2*Math.tan(Math.PI/6),l=a.path("M"+(d.x-l/2)+" "+(d.y+m)+"h"+l+"l"+-l/2+" "+-k+"Z"),k=l.clone().attr({"stroke-width":0,
fill:"black"});l.attr({"stroke-width":c.borderLine,fill:"white"});c=.32*b;m=.1*b;c=a.path("M"+(d.x-c/2)+" "+(d.y-c/2)+"l"+c+" "+c+"M"+(d.x-c/2)+" "+(d.y+c/2)+"l"+c+" "+-c);c.attr({"stroke-width":m,"stroke-linecap":"round"});m=c.clone().attr({color:"white"});a=a.circle(d.x,d.y,.13*b).attr({"stroke-width":.04*b,fill:"white"});b=a.clone().attr({fill:"black"});return{CR_B:f,SQ_B:g,TR_B:l,MA_B:m,CR_W:e,SQ_W:h,TR_W:k,MA_W:c,CR_E:e,SQ_E:h,TR_E:k,MA_E:c,TW:a,TB:b}},markCurrentMove:function(a){var b=this.boardSetting.gridWidth;
...
b.color!=a.color&&yogo.logWarn("wrong color: ("+a.x+","+a.y+")","show move number");var c=a.moveNumber,d=b["stone"+b.color],e=d.data("mn_"+c);if(e)a.current&&(this.unmarkCurrentMoveNumber(),e.attr({fill:"red"}),this.currentMoveNumberElement=e),e.show(),this.moveNumberElements.push(e);else{e="black";"B"==b.color&&(e="white");var f=e;a.current&&(e="red");var g=this.boardSetting.moveNumbers.fontSize;9<c?g-=1:99<c&&(g-=2);var h=this.coordinateManager.boardCoorToViewBoxCoor(a),e=this.paper.text(h.x,h.y,
...
Game.prototype={diffPosition:function(a,b){for(var c=[],d=[],e=[],f=0;f<this.boardSize;f++){var g=a[f],h=b[f];if(g!==h)for(var l=0;l<this.boardSize;l++){var k=g[l],m=h[l];if(k!==m&&(k||m)){var n=!1,p=!1;m?k?k.color!=m.color&&(p=n=!0):p=!0:n=!0;k={x:f,y:l};n&&c.push(k);p&&("B"==m.color?e.push(k):d.push(k))}}}return{stonesToRemove:c,stonesToAddB:e,stonesToAddW:d}},playNode:function(a){if(!a)return!1;var b=this.board,c=this.curNode,d=this.curNode=a;a=!0;if(d.status.positionBuilt){if(d==c)return!1;if(c.nextNode!=
...
a.y);b&&(d.push({x:a.x,y:a.y,color:b}),this.setPointColor(a.x,a.y,null),this.buildPositionOnly||this.board.removeStone(a))}.bind(this),f=0;f<c.length;f++){var g=c[f];if(g.coorFrom)for(var g=yogo.evaluatePointRange(g.coorFrom,g.coorTo),h=0;h<g.length;h++)e.call(this,g[h]);else e.call(this,g)}b.position=this.getFinalPosition();b.status.positionBuilt=!0;return a}};function GameViewer(a){this.game=a}
...
d=a.status.move||a.status.pass?[1,1,1]:[0,0,0];a.numbers={globalMoveNumber:d[0],displayMoveNumber:d[1],variationMoveNumber:d[2]};a.move.MN&&(a.numbers.displayMoveNumber=a.move.MN);a.status.variationFirstNode&&(a.numbers.variationMoveNumber=a.status.move||a.status.pass?1:0);if(a.variations){for(var d=a.variations,h=[],l=0;l<d.length;l++){var k=d[l].nodes[0];k.status.move&&h.push(k.move.B||k.move.W)}a.branchPoints=h}a.id="n"+b.seq++;c.nodeMap[a.id]=a;a.belongingVariation.realGame&&((d=a.numbers.globalMoveNumber)&&
...
[],d=b.x,e=a.y,f=b.y,g=a.x;g<=d;g++)for(var h=e;h<=f;h++)c.push({x:g,y:h});return c}};